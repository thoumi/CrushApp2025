...
Cette overview décrivait à l'origine une migration monolithe → microservices
qui est aujourd'hui **réalisée** dans le code (Core API, ChatbotService,
MediaService, API Gateway existent). Les documents suivants couvrent ce qui
existe réellement plutôt qu'une suite numérotée jamais écrite :

1. **[00-OVERVIEW.md](00-OVERVIEW.md)** ← Vous êtes ici
2. **[ADR-0001 — Fondations Clean Architecture](adr/0001-clean-architecture-foundations.md)** — décision d'architecture actuelle
3. **[Audit SOLID Phase 1](solid-audit-phase1.md)** — constats et corrections en cours
4. **[Docker Compose](03-DOCKER-COMPOSE-COMPLETE.md)** — orchestration multi-services
5. **[Roadmap complète](../roadmap.md)** — event-driven (Phase 3), observabilité (Phase 6), CI/CD complet (Phase 7) : planifiés, pas encore implémentés
...
1. **Lire l'ADR actuel** → [ADR-0001 — Fondations Clean Architecture](adr/0001-clean-architecture-foundations.md)
2. **Suivre l'avancement** → [Roadmap](../roadmap.md)
3. **Voir les constats en cours** → [Audit SOLID Phase 1](solid-audit-phase1.md)
